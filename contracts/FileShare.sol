...
contract FileShare {
    ...
    event FileAccessed(bytes32 indexed proof, address indexed accessedBy, uint256 timestamp, string ipfsHash);
    event UnauthorizedAccess(bytes32 indexed proof, address indexed attemptedBy, string ipAddress, uint256 timestamp, string ipfsHash);
    ...
    function logAccess(string memory _ipfsHash, address user) public {
        emit FileAccessed(keccak256(abi.encodePacked(_ipfsHash, user)), user, block.timestamp, _ipfsHash);
    }

    function logUnauthorizedAccess(string memory _ipfsHash, address user, string memory ipAddress) public {
        emit UnauthorizedAccess(keccak256(abi.encodePacked(_ipfsHash, user)), user, ipAddress, block.timestamp, _ipfsHash);
    }
